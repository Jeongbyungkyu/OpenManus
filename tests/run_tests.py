...
class TestStuckStateHandling(unittest.TestCase):
    ...
    async def _test_verify_intervention_needed_deeply_stuck(self):
        ...
        self.agent.llm.ask.return_value = MagicMock(
    ...
    async def _test_handle_stuck_state_with_intervention_needed(self):
        ...
        with patch.object(
            self.agent, "_verify_intervention_needed", AsyncMock(return_value=True)
        ):
            ...
            with patch("builtins.input", return_value="/continue"):
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    self.assertEqual(
                        len(option_calls), 0, "Options text should not be displayed"
                    )
                ...
                with patch("builtins.input", return_value="/topic=machine learning"):
                    with patch("builtins.print"):
                        ...
                        for msg in self.agent.memory.messages:
                            if (
                                msg.role == "system"
                                and "User provided parameter: topic=machine learning"
                                in msg.content
                            ):
                        ...
                        self.assertTrue(
                            system_msg_found,
                            "System parameter message not found in memory",
                        )
                        ...
                        for msg in self.agent.memory.messages:
                            if (
                                msg.role == "user"
                                and "I'm interested in topic: machine learning"
                                in msg.content
                            ):
                                user_msg_found = True
                                break
                        self.assertTrue(
                            user_msg_found, "User parameter message not found in memory"
                        )
