...
class BaseAgent(BaseModel, ABC):
    ...
    async def handle_stuck_state(self):
        ...
        if deeply_stuck:
            logger.warning(
                "Agent is deeply stuck in a loop and cannot proceed. Pausing for user intervention."
            )
        else:
            logger.warning(
                "Agent is stuck and requires guidance to proceed. Pausing for input."
            )
        ...
        if user_input.startswith("/"):
            ...
            if user_input == "/continue":
                ...
                self.next_step_prompt = (
                    "Starting with a fresh approach based on the initial request."
                )
                ...
                self.update_memory(
                    "system", f"User provided parameter: {param}={value}"
                )
    ...
    def is_stuck(self) -> bool:
        ...
        if question_pattern:
            question_count = sum(
                ...
                and self._get_question_pattern(msg.content) == question_pattern
            )
    ...
    def _process_special_command(self, request: str) -> str:
        ...
        if command == "continue" or command == "proceed":
            ...
            result = (
                "Proceeding with default values. Agent has been instructed to continue."
            )
            self._reset_stuck_detection()
            return result
    ...
    def _reset_stuck_detection(self):
        ...
        self.next_step_prompt = (
            "Process user command and proceed without asking the same questions again."
        )
